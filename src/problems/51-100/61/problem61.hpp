//
// Created by Alex Beccaro on 21/01/18.
//

#ifndef PROJECT_EULER_PROBLEM61_HPP
#define PROJECT_EULER_PROBLEM61_HPP

#include <map>
#include <vector>

using uint = unsigned int;
using std::vector;
using std::map;
using std::pair;
using couple = pair<uint, uint>; // (number of sides of figure, figurate number)

namespace problems {
    class problem61 {
    private:
        /**
         * Search for a cycle of length = 'length' in given map starting from given solution and testing entry as next element.
         * @param m The map with data
         * @param entry The next element entry to test
         * @param sol The partial solution found. Final solution will be stored here.
         * @param length The length of desired solution
         * @return
         */
        static bool find_cycle(const map<couple, vector<couple>>& m,
                               const pair<couple, vector<couple>>& entry,
                               vector<couple>& sol, uint length);
    public:
        /**
         * Solves problem
         * @param n The size of required set
         * @return The solution
         */
        static uint solve(uint size = 6);
    };
}


#endif //PROJECT_EULER_PROBLEM61_HPP
