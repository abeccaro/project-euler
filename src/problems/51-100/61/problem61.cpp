...
namespace problems {
    bool problem61::find_cycle(const map<couple, vector<couple>> &m,
                               ...
                               vector<couple> &sol, uint length) {
        uint n = entry.first.first; // number of sides of figure
    ...
    uint problem61::solve(uint size) {
        map<couple, vector<couple>> m; // number -> list of numbers that can follow key in a cycle

        for (uint i = 3; i < 3 + size; i++) {
            // generate figurate numbers with 4 digits
            figurate_number<uint> s(i);
            vector<uint> numbers = s.get_while([](uint n) { return n < 10000; },
                                                  [](uint n) { return n >= 1000; });
